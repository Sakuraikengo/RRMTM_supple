...
# weatherDataList <- list.files(path = "raw_data/", pattern = "WeatherData_ALRC", 
#                               full.names = T, recursive = T)
# for (weatherDataInd in 1:length(weatherDataList)) {
#   # weatherDataInd <- 1
#   weatherData <- read.csv(weatherDataList[[weatherDataInd]])
#   
#   sowingDay <- as.factor(sowingDayList[[weatherDataInd]])
#   destructiveDay <- as.factor(DestructiveDayList[[weatherDataInd]])
#   startInd <- match(x = sowingDay, table = weatherData$Date)
#   endInd <- match(x = destructiveDay, table = weatherData$Date)
#   
#   rainSum <- sum(weatherData[startInd:endInd, "Rainfall.mm"])
#   print(rainSum)
# }
